...
__device__ void
my_search_block_up( ArrayT array, position_t size, KeyT val, position_t* num_up )
{
  ...
  if (tid == 0) {
    ...
    printf("arr[n-1]: %d arr[n-2] %d val %d\n", (int)getKey(array, size-1),
	   (int)getKey(array, size-2), (int)val);
  }
  __syncthreads();
...
template < class KeyT, class ArrayT, uint bsize >
__device__ void
old_search_block_up( ArrayT array, position_t size, KeyT val, position_t* num_up )
{
  __shared__ KeyT shared_array[ bsize + 1 ];
  __shared__ position_t left;
  __shared__ position_t right;
...
template < class KeyT, class ArrayT, uint bsize >
__device__ void
my_search_block_down( ArrayT array, position_t size, KeyT val, position_t* num_down )
{
  __shared__ KeyT shared_array[ bsize + 1 ];
  __shared__ position_t left;
  __shared__ position_t right;
  ...
  if (tid == 0) {
    printf("bid:%d tid:0 step:%d size:%d\n", (int)blockIdx.x, (int)step, (int)size);
    printf("arr[n-1]: %d arr[n-2] %d val %d\n", (int)getKey(array, size-1),
  	   (int)getKey(array, size-2), (int)val);
  }
  __syncthreads();
  while ( step > 1 && ( right - left ) > 1 )
  {
    ...
    else if ( tid <= n_steps )
    {
      ...
      if ( pos < size && ( right - pos + step ) >= 1 )
      {
        ...
        printf("bid:%d tid:%d sa:%d pos:%d arr:%d\n", (int)blockIdx.x, (int)tid,
	       (int)shared_array[tid], (int)pos, (int)getKey(array, pos));
      }
    }
    __syncthreads();
    if ( ( tid < n_steps ) && pos < size && ( ( right - pos ) >= 1 ) && ( shared_array[ tid ] < val )
      && ( (tid + 1)>=n_steps || pos1>=size ||  shared_array[ tid + 1 ] >= val ) )
    {
      left = pos;
      right = min( pos1, right );
      printf("bid:%d good tid:%d sa0:%d sa1:%d l:%d r:%d\n", (int)blockIdx.x,
	     (int)tid, (int)shared_array[tid], (int)shared_array[tid+1],
	     (int)left, (int)right);
    }
    __syncthreads();
  }
  ...
  {
    *num_down = right;
    printf("mysbd Kernel block: %d\tnum_up: %d\n", (int)blockIdx.x, (int)right);
    printf("mysbd bid: %d\tleft: %d\tright: %d\n", (int)blockIdx.x, (int)left, (int)right);
  }
}
...
template < class KeyT, class ArrayT, uint bsize >
__global__ void
my_search_multi_down_kernel( ArrayT* subarray, KeyT* val_pt, position_t* num_down, int bid)
{
  KeyT val = *val_pt;
  my_search_block_down< KeyT, ArrayT, bsize >( subarray[ bid ], subarray[ bid ].size, val, &num_down[ bid ] );
  if ( threadIdx.x == 0 )
  {
...
template < class KeyT, class ArrayT, uint bsize >
__device__ void
old_search_block_down( ArrayT array, position_t size, KeyT val, position_t* num_down )
{
  __shared__ KeyT shared_array[ bsize + 1 ];
  __shared__ position_t left;
  __shared__ position_t right;
